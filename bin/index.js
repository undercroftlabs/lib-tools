...
var BuildCommand = {
  ...
  async handler() {
    try {
      console.log("\u{1F527} Running tsup...");
      const tsupConfig = configExists("tsup.config.js") ? "tsup.config.js" : path.resolve(__dirname2, "../../config/tsup.config.js");
      await run("tsup", ["--config", tsupConfig]);
      if (configExists("rollup.config.js")) {
        console.log("\u{1F4E6} Running rollup...");
        await run("rollup", ["-c"]);
      } else {
...
// src/cli/dependencies-check.ts
init_esm_shims();
var import_semver = __toESM(require_semver2(), 1);
import depcheck from "depcheck";

// src/cli/core/header-builder.ts
init_esm_shims();
var colors = {
  success: "\x1B[32m",
  warning: "\x1B[33m",
...
function buildHeader(title, hasError) {
...
// src/cli/core/read-package-json.ts
init_esm_shims();
import fs2 from "fs";
var readPackageJson = (path5) => {
  const fileContent = fs2.readFileSync(path5, "utf8");
  const packageJson = JSON.parse(fileContent);
  return packageJson;
};

// src/cli/core/get-project-paths.ts
init_esm_shims();
...
var getProjectPaths = () => {
  ...
  return {
    rootPath: root,
    packageJsonPath: path2.join(root, "package.json"),
    nodeModulesPath: path2.join(root, "node_modules")
  };
};

// src/cli/dependencies-check.ts
var DependenciesCheckCommand = {
...
async function runDepcheck() {
  ...
  const unusedDependencies = Object.entries(
    productionDeps
  ).reduce((deps, [name, version]) => {
    const normalizedVersion = import_semver.default.coerce(version)?.version ?? "";
    deps[name] = {
      name,
...
var LicensesCheckCommand = {
  ...
  async handler() {
    try {
      ...
      const licenseLines = await new Promise((resolve, reject) => {
        lcc.check(
          packageJsonPath,
          nodeModulesPath,
          (err, _passe, output) => {
      ...
      const incompatible = intersect(
        productionDeps,
        licenseLines.filter((line) => line.match(incompatibleRegex)).map((line) => line.split("@")[0])
      );
      const getLicenseInfo = (name) => licenseLines.find((line) => line.includes(name)) ?? "";
      const hasIssues = incompatible.length > 0;
      const header = buildHeader(
        hasIssues ? "License Issues Found In Production Packages." : "No License Issues Found In Production Packages.",
        hasIssues
      );
      console.log(header);
      productionDeps.map(getLicenseInfo).filter((line) => line.trim()).forEach((line) => console.log(line));
...
var licenses_check_default = LicensesCheckCommand;

// src/cli/size-check.ts
init_esm_shims();
import path3 from "node:path";
import fs3 from "node:fs";
var configExists2 = (filename) => fs3.existsSync(path3.resolve(process.cwd(), filename));
var SizeCheckCommand = {
  command: "check:size",
  describe: "Check bundle size limits using size-limit",
  builder: {},
  async handler() {
    try {
      console.log("\u{1F4E6} Running size-limit...");
      const configFile = [
        ".size-limit.cjs",
        ".size-limit.js",
        ".size-limit.mjs",
...
// src/cli/prettier-check.ts
init_esm_shims();
import path4 from "node:path";
import fs4 from "node:fs";
var configExists3 = (filename) => fs4.existsSync(path4.resolve(process.cwd(), filename));
var PrettierCheckCommand = {
  command: "check:prettier",
  describe: "Check code formatting using Prettier",
  builder: {},
  async handler() {
    try {
      console.log("\u{1F9F9} Checking formatting with Prettier...");
      const configFile = [
        ".prettierrc.cjs",
        ".prettierrc.js",
        ".prettierrc.json",
...
var CheckCommand = {
  ...
  async handler() {
    ...
    for (const check of checks) {
      console.log(`\u{1F50E} Running ${check.name}...`);
      try {
...
var check_default = CheckCommand;
...
var prepare_default = PrepareCommand;

// src/cli/postpublish.ts
init_esm_shims();
var PostPublishCommand = {
  command: "postpublish",
...
// src/cli/index.ts
yargs(hideBin(process.argv)).scriptName("undercroft-tools").command(build_default).command(TestCommand).command(CoverageCommand).command(TestCICommand).command(check_default).command(DependenciesCheckCommand).command(licenses_check_default).command(SizeCheckCommand).command(PrettierCheckCommand).command(prepare_default).command(PostPublishCommand).command(PreflightCommand).command(PackageCommand).demandCommand().strict().help().parse();
